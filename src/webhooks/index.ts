...
export function registerWebhookHandlers(app: App) {
    ...
    app.webhooks.on("workflow_run", async ({ octokit, payload }) => {
        ...
        try {
            ...
            logger.info(
                `Updated workflow run status to: ${dbStatus}${conclusion ? `, conclusion: ${conclusion}` : ""}`
            );
            ...
            if (action === "completed") {
                if (workflowRun.conclusion === "success") {
                    ...
                    logger.warn(
                        `✗ SDK workflow ${workflowRun.name} completed with conclusion: ${workflowRun.conclusion}`
                    );
                }
            }
        } catch (error) {
